...
export class VacinaService {
  ...
  protected convertDateFromClient(vacina: IVacina): IVacina {
    return Object.assign({}, vacina, {
      criada: vacina.criada?.isValid() ? vacina.criada.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.criada = res.body.criada ? dayjs(res.body.criada) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((vacina: IVacina) => {
        vacina.criada = vacina.criada ? dayjs(vacina.criada) : undefined;
      });
    }
    return res;
  }
